...
"""OpenSlide-backed reading of vendor WSI formats — ingestion only.

This module is used to bake a canonical Zarr pyramid once per slide (see
:mod:`compass.connector.ingest`) and must never be used as a runtime
raster source: OpenSlide and Bio-Formats disagree on MRXS coordinate
interpretation, so OpenSlide-read pixels are only trusted after the
converted raster has been verified against known landmark annotations
...
from ..core import ImageShape, Magnification, Px, PyramidalRaster


#####
class WSI(PyramidalRaster):
    """An extended version of OpenSlide, with more handy methods for dealing
    with microscopy slide images.
